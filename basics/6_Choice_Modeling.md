...
- Validates measurement models developed through EFA and estimates whether observed variables load on hypothesized latent 
factors; CFA typically precedes full SEM
...
- **R functions**: `arules::apriori()` (and other `arules` methods)
...
- **Applications**: product design, pricing strategy, market share prediction, new product forecasting, policy analysis, conjoint analysis (preference / trade-off designs)
- **R functions**: `mlogit::mlogit()`, `apollo::apollo_estimate()` (for advanced choice models)
- **Note**: Marketing Mix Modeling as channel-spend regression lives in Seminar 4; here the focus is discrete choice among alternatives

### 5. Recap: Statistical Tests and Group Comparisons
- **Parametric**: t-test (one-sample, independent, paired), ANOVA (one-way, two-way, repeated measures) when assumptions met
- **Non-parametric**: Mann-Whitney U (Wilcoxon rank-sum), Wilcoxon signed-rank (paired), Kruskal-Wallis when assumptions violated
- **Categorical**: Chi-square test of independence, Fisher's exact test (small samples)
...
- **Hierarchical / mixed-effects**: nested data (customers in regions, repeated measures)
- **GLM**: count data and other exponential-family / non-normal outcomes
- **Bayesian**: when prior knowledge exists, small samples

### 7. Recap: Complexity Reduction and Grouping
- **Scaling / normalisation**: prepare features before PCA, MDS, and distance-based clustering
- **PCA**: dimensionality reduction, preparation for clustering
- **EFA**: scale development, latent construct identification (then CFA / SEM above)
- **MDS**: perceptual mapping, brand positioning
- **Clustering (unsupervised)**: customer segmentation (k-means, hierarchical, DBSCAN)
- **Classification (supervised)**: assign observations to predefined categories (Naive Bayes, Random Forest)
