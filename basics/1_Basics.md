## Seminar 1

### 1. Introduction & Organizational aspects
- seems self-explanatory

### 2. Course Goals
- Choose methods adequately to the objectives of a research project <- refreshing statistics basics and learning when and how to apply different statistical methods
- Collect, store, process and analyze data according to high standards <- data preprocessing - data objects and types, reading data and writing results, working with NAs and outliers, EDA
- Conduct empirical business research and solve managerial problems using modern analytic software tools <- use actual datasets from business to make decisions with R
- Select relevant statistical methods and apply them correctly to marketing problems <- choose adequate to given problem method

### 3. Data-driven Decision Making
- How it may be helpful for business
- Examples of real-world tasks
- Introduction of Hypothesis Testing approach

### 4. Population and Sample
- Ideas behind sampling
- Sampling techniques

### 5. Understanding your data
- Types
- Descriptive statistics
- Distributions

### 6. Experiment Design and Statistical Significance
- Steps of experiment design
- Short notion on p-value, significance level, power of the test, etc.

### 7. R Environment Setup
- Good if 100% of audience is ready to go R
- Otherwise - will have to split into groups with at least 1 R user
- [R setup](https://rstudio-education.github.io/hopr/starting.html)
- See attached 'install_pack.R' file for all needed packages
