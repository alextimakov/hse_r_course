---
title: "Seminar 6: Choice Modeling (Final)"
...
Requires: dplyr, ROSE, randomForest, caret.  
...
Aggregate unique CustomerID per Country at first. Chi-square: test if customer counts differ across countries (equal as H0). Works well for frequencies with large enough sample sizes. p < 0.05 suggests significant difference; we could also see that UK has way more customers.
...
# If we have counts of returns per method, chi-square on counts. This one seems more reliable
...
```{r q2-rose}
# Let's try resample a bit with ROSE. Much better specificity with a huge loss to accuracy and F1
...
pred_churn_glm_balanced <- factor(ifelse(pred_churn_balanced > 0.5, 1, 0),
                                  levels = c(0, 1))
show_results(test_data, pred_churn_glm_balanced, "Churn")

# Let's try RF + ROSE. Slightly better results, adequate F1 and Specificity
...
Use total_spent to predict cluster, train-test split. Will just go with RF as the likely best tool; worked very well (maybe even too good).
